...
function UserProvider({ children }) {
  ...
  const fetchLists = async () => {
    ...
    const res = await axios.get(
      `http://localhost:8000/user/user_lists/${userId}`
    );
  ...
  const updateList = async (list, action) => {
    if (action === "add") {
      const res = await axios.post(
        `http://localhost:8000/user/new_list/${userId}`,
        { list }
      );
      return res;
    } else if (action === "delete") {
      const res = await axios.delete(
        `http://localhost:8000/user/delete_list/${userId}`,
        { params: { list } }
      );
      return res;
    }
  ...
  const updateListItems = async (list, action, item) => {
    if (action === "add") {
      const res = await axios.post(
        `http://localhost:8000/user/new_item/${userId}`,
        { list, item }
      );
      return res;
    } else {
      ...
      const res = await axios.delete(
        `http://localhost:8000/user/delete_item/${userId}`,
        { params: { list, itemID } }
      );
      return res;
    }
